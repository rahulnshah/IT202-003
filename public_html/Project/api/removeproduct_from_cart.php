...
if (isset($_POST["id"]) && isset($_POST["desired_quantity"])) { //need to check this, becuz user can easily navigate to this file 
    ...
    if($isValid){
        ...
        if($quantity > 1)
        {
            ...
            $sql = "UPDATE Cart SET desired_quantity=:desired_quantity - 1 WHERE id=:id";
            $stmt= $db->prepare($sql);
            try{
                $data = [":id" => $cart_id, ":desired_quantity" => $quantity];
                $stmt->execute($data);
                $response["message"] = "Decremented quantity by 1";
            }
            catch(PDOException $e) {
                flash("Error decrementing desired_quantity of cartId $cart_id: " . var_export($e->errorInfo, true), "warning");
            }
        }
        else
        {
